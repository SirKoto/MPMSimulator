...
public:
	...
	virtual unsigned int dumpPositions(float* positions) const;

	// Returns the number of particles. Dumps the position of each particle [0,1]
	virtual unsigned int dumpPositionsNormalized(float* positions);

	// Main function. Make a time advance of dt
	virtual void step(float dt);

	// Add new particle. The position can be from [0, grid_size]
	virtual void addParticle(const glm::vec3& pos, const glm::vec3& v = glm::vec3(0), int material = 0);

	// Add new particle. The position can be from [0, 1]
	virtual void addParticleNormalized(const glm::vec3& pos, const glm::vec3& v = glm::vec3(0), int material = 0);

	// Remove all data from the pysicsGrid
	virtual void clearPhysics();

	// Create flat physics surface at height [0,1]
	virtual void setPhysicsFlat(float height);
	...
	virtual void setPhysicSlopes(float height, float initialH, float holeWidth, int depth = 1);

	// Create vertical wall in the XY plane
	virtual void setPhysicsZWall(float zmin, float zmax, int depth = 1);

	// Get young modulus from material in position pos
	float getYoung(int pos = 0) const { return v_properties[pos].young; }

	float getNu(int pos = 0) const { return v_properties[pos].nu; }

	float getHardening(int pos = 0) const { return v_properties[pos].hardening; }

	float getVolume(int pos = 0) const { return v_properties[pos].volume; }

	float getMass(int pos = 0) const { return v_properties[pos].mass; }

	float getPlasticity(int pos = 0) const { return static_cast<float>(v_properties[pos].plasticity); }

	float getT_C(int pos = 0) const { return v_properties[pos].t_c; }

	float getT_S(int pos = 0) const { return v_properties[pos].t_s; }

	HYPERELASTICITY getMode() const { return mode; }
	// Get the number of materials in the system actually
	int getNumMaterials() const { return static_cast<int>(v_properties.size()); }

	// add a new material
	virtual int addNewMaterial(float young, float nu, float hardening, float volume, float mass, bool plasticity, float t_c, float t_s);
