...
if (idx < d_numParticles) {
	...
	/*else if (mode == Simulator_3D::HYPERELASTICITY::COROTATED)
	{
		assert(false);
		mat3 U, V;
		vec3 s;
		...
		mat3 R = mm::mul_trans(U, V);

		mat3 PF_t = mm::sub(F, R);
		PF_t = mm::mul_trans(PF_t, F);
		mm::mul_in(&PF_t, 2.0f * mu);
		mm::add_to(&PF_t, (lambda * (J - 1.0f) * J));
...
__global__ void P2G_Corrotated(const float dt) {
	unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x;
